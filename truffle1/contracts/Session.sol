...
contract Session {
    ...
    modifier onlyAdmin(){
        require(msg.sender == creator, "Admin only !");
        _;
    }
    ...
    function startSession(uint _timeOut) public onlyAdmin{
        require(state == State.CREATED || state == State.STOPPED, "Can't start session !");
    ...
    function stopSession() public onlyAdmin {
        require(state == State.CREATED || state == State.ONGOING || state == State.CLOSED, "Can't stop session !");
        stateBefore = state;
        state = State.STOPPED;
        timeStop = now;
    }
